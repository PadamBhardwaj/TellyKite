...
export const Reseller = ({ history }) => {
    ...
    var len = 0
    if (customers) {
        var len = customers.customers.length;
    }
    ...
    return (
        <>
            ...
            <div className={Styles.List}>
                <h5>Customer:</h5>
                {(!loading && customers) && customers.customers.map((customer, index) => <Customer key={index} num={index} name={customer.username} email={customer.email} />)}
            </div>
